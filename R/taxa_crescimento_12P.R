...
# 12 pontos no gráfico

taxa_crescimento_12P <- function(x){
  glm <- list()
  for(i in names(elaborar_taxa_cresc_12P)[-1]){
    glm[[i]] <- lm(get(i) ~ ano, elaborar_taxa_cresc_12P)
  }
  ...
  df_taxa <- cbind(taxa, descriçãoo = names(glm))
  ...
  df_taxa2 [1:12,1]
  df_taxa2$nova_coluna <- df_taxa2 [1:12,1]

  df_taxa2 [13:24,1]
  df_taxa2$nova_coluna2 <- df_taxa2 [13:24,1]

  df_taxa2 <- df_taxa2[, -1]
  df_taxa2 <- df_taxa2[-c(13:24),]

  colnames(df_taxa2)[1] <- "tx_qtd"
  colnames(df_taxa2)[2] <- "tx_preco"

  pais <- c("País 1 (Período 1)", "Pais 1 (Período 2)",	"País 1 (Período 3)",	"País 1 (Período 4)", "País 2 (Período 1)", "País 2 (Período 2)",	"País 2 (Período 3)",	"País 2 (Período 4)", "País 3 (Período 1)", "País 3 (Período 2)",	"País 3 (Período 3)",	"País 3 (Período 4)")
  ...
  ggplot(df_taxa2, aes(x=tx_preco, y=tx_qtd, color=pais, shape=pais)) +
    ...
    labs(title = "Taxa de crescimento de castanha com casca  \n - principais exportadores (1998-2017)", x = "Taxa de crescimento em preço (%)", y = "Taxa de crescimento em quantidade (%)")+
    ...
    scale_shape_manual(values = c(16, 17, 15, 18, 8, 7,3,4, 0, 14, 12,20))
